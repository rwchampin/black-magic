...
import psrdnoise from '~/shaders/psrdnoise3.glsl'
const { randFloat: rnd, randFloatSpread: rndFS } = MathUtils

const defaultConfig = {
  gpgpuSize: 256,
  colors: [0x5D20C9, 0x20C997],
  color: 0x5D20C9,
  coordScale: 1.5,
...
export const usePointerParticles = (params) => {
  const config = { ...defaultConfig, ...params }
  const { scene, renderer, camera } = config.nuxtApp.$core
  ...
  three({
    ...
    beforeRender({ width, wWidth, wHeight, clock, pointer }) {
      ...
      uMouse.value.lerp(mouseTarget, 0.5)
